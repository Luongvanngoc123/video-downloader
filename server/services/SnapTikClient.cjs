...
class SnapTikClient {
  constructor(config = {}) {
    ...
    // Add proxy support if HTTPS_PROXY or HTTP_PROXY environment variable is set
    const proxyUrl = process.env.HTTPS_PROXY || process.env.HTTP_PROXY || config.proxy;
    if (proxyUrl) {
      console.log('SnapTikClient using proxy:', proxyUrl);
      baseConfig.httpsAgent = new HttpsProxyAgent(proxyUrl);
      baseConfig.httpAgent = new HttpsProxyAgent(proxyUrl);
    }
    
    this.axios = axios.create(this.config = baseConfig);
  }
